 ...
 * VERSION: 9.4.07 based on 9.4.04 with fix for resetting variables on backswing.
 * ===================================================================================== */
...
void sendBatteryUpdate() {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        // (Make sure delay(1) is removed here to keep it fast!)
    }
...
void savePendingToFlash() {
    ...
    for (uint16_t i = 0; i < pendingStrikeCount; i++) {
        NRF_WDT->RR[0] = 0x6E524635; // Feed the dog during heavy flash writes!
...
void enterGameModeSleep() {
    ...
    while (!imuAwakeFlag) {
        NRF_WDT->RR[0] = 0x6E524635; // Feed the dog during SysTick wakeups!
...
void updateKinematics(unsigned long currentMillis, unsigned long currentMicros, bool connected) {
    if (currentMicros - previousImuMicros >= IMU_INTERVAL_MICROS) {
        ...
        float calRoll = asin(sinRoll) * (180.0f / PI);
       ...
       // --- THE FIX: MATCH MODE AUTO-ADDRESS TARE ---
        ...
        if (isGameMode && currentGameSubState == GAME_READY_PULSE) {
            ...
            if (stillnessCount > 1500 && fabs(calRoll) < 12.0f) {
                ...
                pitchOffset = atan2(2.0f * (f_q0*f_q1 + f_q2*f_q3), 1.0f - 2.0f * (f_q1*f_q1 + f_q2*f_q2)) * (180.0f / PI);
        ...
        // --- NEW: Reset swing trackers when crossing back behind the ball ---
        if (prevPitchRads >= 0.0f && pitchRads < 0.0f) {
            minPitch = 0.0f;
            maxPitch = 0.0f;
            zVelocity = 0.0f;
            topOfBackswingTime = 0;
        ...
        if (pitchRads < minPitch) {
            ...
            // Clear the BLE payload for the new swing
            currentAppliedForce = 0;
        }

        // --- NEW METRIC: PEAK ANGULAR ACCELERATION (Noise-Filtered) ---
        static float maxAngularAccel = 0.0f;
        static float smoothedAccel = 0.0f; // NEW: The vibration filter

        // 1. Reset the trackers at the top of the backswing
        if (pitchRads < minPitch) {
            maxAngularAccel = 0.0f;
            smoothedAccel = 0.0f; // Reset filter for the new swing
        }

        // 2. Track the highest acceleration spike during the forward downswing
        if (omegaSigned > 0.0f && pitchRads < 0.0f) {
            
            // Raw derivative (Highly noisy due to 2ms actualDt)
            float raw_accel = (currentOmegaMag - fabs(prevOmegaSigned)) / actualDt;
            
            // Apply EMA Filter: 10% new reading, 90% history to smooth out 500Hz jitter
            smoothedAccel = (smoothedAccel * 0.90f) + (raw_accel * 0.10f);
            
            // Track the highest point of the smoothed wave
            if (smoothedAccel > maxAngularAccel) {
                maxAngularAccel = smoothedAccel;
            }
        }

        // 3. Evaluate and log at the strike plane
        if (topOfBackswingTime > 0 && omegaSigned > 0 && prevPitchRads < 0.0 && pitchRads >= 0.0) {
            
            // Exact max gravity acceleration (alpha = (g / r) * sin(theta))
            float exact_gravity_accel = (9.81f / radius_m) * sin(fabs(minPitch));
            
            // Pure muscular effort (True peak minus max gravity pull)
            float extra_effort = maxAngularAccel - exact_gravity_accel; 
            
            // Unscaled, true physical rad/s^2 output
        ...
        if (inImpactWindow) {
            ...
            if (elapsed <= 25) {
                ...
                if (isArmed || (passedDirection && passedDwell)) {
                    ...
                    // Neutralized path force slot (dead padding to preserve 32 bytes)
                    sp.undefined_0 = 0;
...
void setup() {
    ...
    // Lock unused external I2C pads to prevent parasitic noise drain
    ...
    // --- WATCHDOG ADDITION ---
    NRF_WDT->CONFIG = 0x01;       // FIX: Set back to 0x01 so it protects against I2C hangs!
    NRF_WDT->CRV = 32768 * 5;     
    NRF_WDT->RREN = 0x01;         
    NRF_WDT->TASKS_START = 1;     

    // ... [The rest of your setup continues normally below] ...
    // FIX: Safely power the internal Sense IMU to prevent latch-up short
    pinMode(IMU_INT_PIN, INPUT_PULLDOWN);
    pinMode(PIN_LSM6DS3TR_C_POWER, OUTPUT);
    digitalWrite(PIN_LSM6DS3TR_C_POWER, HIGH);
    delay(250); // Give power time to stabilize before IMU begin
...
void loop() {
    ...
    NRF_WDT->RR[0] = 0x6E524635;  // Feed the watchdog to prevent hardware reboot
    ...
    if (!connected) {
        if (!isGameMode) {
            ...
            if (currentMillis - lastActivityTime > 180000) {
    ...
    if (connected && !inImpactWindow) {
       ...
       if (currentMillis - previousBleMillis >= currentBleInterval) {
            ...
            if (currentOmegaMag > 2.0) {
                ...
                dynR = (dynamic_ac * 9.81) / (currentOmegaMag * currentOmegaMag);
            }
...
void enterSafeChargingMode() {
    ...
    while (NRF_POWER->USBREGSTATUS & 1) {
        NRF_WDT->RR[0] = 0x6E524635; // Keep the watchdog timer happy
        delay(250); 
    }

    // 5. The USB cable was just unplugged!
    // Perform a clean system reboot to bring Bluetooth and sensors back to life instantly.
    NVIC_SystemReset();
}
...
void goToDeepSleep() {
    ...
    // ---------------------------------------------------------
    // STEP 4: I2C & Interrupt Pin Lockdown - CYAN
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // STEP 4.5: Kill Serial and ADC Hardware
    // ---------------------------------------------------------
    Serial.end();
    ...
    // ---------------------------------------------------------
    // STEP 5: Peripheral Power Kills - BLUE
    // ---------------------------------------------------------
    //setLEDColor(0, 0, 255);
    //delay(100);
    ...
    // ---------------------------------------------------------
    // STEP 6: QSPI & Nordic Errata - RED
    // ---------------------------------------------------------
    
    NRF_QSPI->TASKS_DEACTIVATE = 1;
    NRF_QSPI->ENABLE = 0;

    for (int i = 0; i < 32; i++) { NRF_P0->PIN_CNF[i] &= ~(GPIO_PIN_CNF_SENSE_Msk); }
    for (int i = 0; i < 16; i++) { NRF_P1->PIN_CNF[i] &= ~(GPIO_PIN_CNF_SENSE_Msk); }
    ...
    // ---------------------------------------------------------
    // STEP 7: Final LED Kill & SYSTEMOFF
    // ---------------------------------------------------------
    // If it survives to here, it will go DARK and sleep successfully.
    ...
    // --- NEW: Scrub all pending interrupts so SYSTEMOFF doesn't abort ---
    for (int i = 0; i < 64; i++) {
