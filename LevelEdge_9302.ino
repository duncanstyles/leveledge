 ...
 * FILE: LevelEdge_9302.ino
 * VERSION: 9.3.02 (Double-Tap Wake, Stillness Tare, Zero-Cross Coach + 5-Gate Kinematics)
 * ===================================================================================== */
            ...
            if (!led_guidance_enabled) {
                ...
                switch(currentGameSubState) {
                    ...
                    case GAME_READY_PULSE:
                        // Sharp Blinking Green (ON for 100ms, OFF for 400ms) waiting for tare
                        if (currentMillis % 500 < 100) {
                            setLEDColor(0, 255, 0);
                        } else {
                            setLEDColor(0, 0, 0);
                        }
                    ...
                    case GAME_CASTING_COACH:
                        if (castLedLatchState == 2) {
                            ...
                            setLEDColor(255, 0, 0);
                        } else if (castLedLatchState == 3) {
                            // DARK: Perfect Cast (On-Line)
                            setLEDColor(0, 0, 0);
                        } else {
                            // SOLID GREEN: Target Locked / Ready (castLedLatchState == 1)
                            setLEDColor(0, 255, 0);
                        }
                        break;
                }
...
void handleBleCommand(const uint8_t* data, int len, unsigned long currentMillis, unsigned long currentMicros) {
    ...
    if (data[0] == 'R') { 
        ...
        if (NRF_POWER->USBREGSTATUS & 1) {
            // If charging, route to the safe dark mode
            enterSafeChargingMode();
        } else {
            // If on battery, go to true deep sleep
            goToDeepSleep(); 
        }
    }
}
...
void updateKinematics(unsigned long currentMillis, unsigned long currentMicros, bool connected) {
    if (currentMicros - previousImuMicros >= IMU_INTERVAL_MICROS) {
        ...
        // --- DOUBLE-TAP GESTURE DETECTOR (Orientation Independent) ---
        // Uses deltaG to detect sharp taps in ANY direction, ignoring static gravity
        if (deltaG > 0.8 && gyroMag < 100.0) {
            if (currentMillis - lastTapTime > 200) { // 200ms debounce
                // If this is the first tap, or it's been over 1.2s since the last one, reset
                if (tapCount == 0 || (currentMillis - firstTapTime > 1200)) {
                    ...
                    if (tapCount == 2) {
                        if (isGameMode) {
                            if (currentGameSubState == GAME_DORMANT) {
                                ...
                                shotClockEndTime = currentMillis + 30000; 
                            } else {
                                resetMatchModeToDormant();
                            }
                        } 
                        else if (currentAppState == STATE_IDLE || currentAppState == STATE_REVIEW) {
                            // Trick the firmware into running the Trainer 'Arm' logic natively
                            uint8_t armCmd = 'G';
                            handleBleCommand(&armCmd, 1, currentMillis, currentMicros);
                        }
        ...
        if (isGameMode && (currentGameSubState == GAME_ALIGNED_SOLID || currentGameSubState == GAME_CASTING_COACH)) {
            ...
            if (castArmedForZeroCross && prevPitchRads < 0.0f && pitchRads >= 0.0f && omegaSigned > 0.2f) {
                if (fabs(currentTwist_deg) > twistTolerance_deg) {
                    ...
                    castLedLatchState = 3; // LATCH 3: Dark (Perfect Cast)
                }
        ...
        if (isArmed || isGameMode) {
            ...
            // FIX: Added !tapLockout to prevent taps from triggering the impact window
            if (!inImpactWindow && !tapLockout && validImpact && deltaG >= impactThreshold && (currentMillis - lastImpactTime > 500)) {
                inImpactWindow = true;
...
void setup() {
    ...
    // --- WATCHDOG ADDITION ---
    NRF_WDT->CONFIG = 0x01;       // FIX: 0x01 forces WDT to run during sleep/I2C hangs
    NRF_WDT->CRV = 32768 * 5;     // FIX: Increased to 5.0 seconds to cover boot delays
...
void loop() {
    ...
    if (!connected) {
        if (!isGameMode) {
            ...
            // Enter safe dark mode if USB is plugged in to charge the battery.
...
void goToDeepSleep() {
    // 1. HARDWARE OVERRIDE: Kill all PWM peripherals to prevent LED ghosting
    NRF_PWM0->ENABLE = 0;
    NRF_PWM1->ENABLE = 0;
    NRF_PWM2->ENABLE = 0;
    NRF_PWM3->ENABLE = 0;
    
    // 2. Lock the custom pins as standard outputs and drive HIGH (Off)
    pinMode(LEDR, OUTPUT);
    pinMode(LEDG, OUTPUT);
    pinMode(LEDB, OUTPUT);
    digitalWrite(LEDR, HIGH);
    ...
    // 3. Gracefully close BLE connections
    BLE.disconnect(); 
    BLE.end();
    delay(100);
    ...
    // 4. PUT IMU TO SLEEP FIRST (While I2C is still active!)
    myIMU.writeRegister(0x10, 0x00); // Turn off Accelerometer
    myIMU.writeRegister(0x11, 0x00); // Turn off Gyroscope
    
    // 5. KEEP IMU POWER PIN HIGH (Prevents the latch-up short circuit!)
    pinMode(PIN_LSM6DS3TR_C_POWER, OUTPUT);
    digitalWrite(PIN_LSM6DS3TR_C_POWER, HIGH); 
    
    // 6. NOW it is safe to shut down the I2C bus
    Wire.end(); 
