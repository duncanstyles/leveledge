 ...
 * FILE: Firmware.ino
 * VERSION: 5.2.19 (Color=Twist, Strobe=Path LED Logic)
 * ===================================================================================== */
    ...
    switch(currentAppState) {
        ...
        case 4: // LIVE SWING ZONE
            if (!led_guidance_enabled) {
                ...
                bool pathIsGood = (abs(lastPassDevX) <= sweetSpot_cm);
                bool twistIsGood = (abs(lastPassTwist) <= twistTolerance_deg);

                // Base color determined purely by Face Twist
                uint8_t r = twistIsGood ? 0 : 255;
                uint8_t g = twistIsGood ? 255 : 0;
                uint8_t b = 0;

                // Modifier: Strobe the base color if the Swing Path failed
                if (!pathIsGood) {
                    if (currentMillis % 150 < 75) { 
                        setLEDColor(r, g, b); // Flash ON
                    } else {
                        setLEDColor(0, 0, 0); // Flash OFF
                    }
                } else {
                    setLEDColor(r, g, b);     // Solid ON
                }
            }
            break;
        case 5: // FINISHED
    ...
    while (central.connected()) {
      ...
      if (commandChar.written()) {
        ...
        if (len > 0) {
            ...
            else if (data[0] == 67 && len >= 9) { // 'C' Configuration Sync
      ...
      if (currentMicros - previousImuMicros >= IMU_INTERVAL_MICROS) {
        ...
        if (isCalibratingBias) {
            ...
            if (biasSamples >= 1000) {
                ...
                lastPassDevX = 0.0; liveDevX = 0.0; lastPassTwist = 0.0;
        ...
        if (isArmed) {
            ...
            // RECORDING THE FORWARD SWING IMPACT DEVIATION & TWIST
            if (prevPitchRads < 0.0 && pitchRads >= 0.0) { 
                lastPassDevX = liveDevX; 
                lastPassTwist = filter.getYaw() - yawOffset;
                // Handle potential circular wraparound
                if (lastPassTwist > 180.0) lastPassTwist -= 360.0;
                if (lastPassTwist < -180.0) lastPassTwist += 360.0;
            }
