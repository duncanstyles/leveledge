 ...
 * FILE: Firmware.ino
 * VERSION: 5.2.17 (Snapshot LED Guidance)
 * ===================================================================================== */
    ...
    switch(currentAppState) {
        ...
        case 4: // LIVE SWING ZONE
            if (!led_guidance_enabled) {
                ...
                // FIX: LED now references the snapshot taken at the bottom of the forward swing (lastPassDevX)
                // instead of the continuously flickering real-time position (liveDevX).
                if (abs(lastPassDevX) <= sweetSpot_cm) {
                    setLEDColor(0, 255, 0);
                } else {
                    setLEDColor(255, 0, 0);
                }
    ...
    while (central.connected()) {
      ...
      if (currentMicros - previousImuMicros >= IMU_INTERVAL_MICROS) {
        ...
        if (isArmed) {
            ...
            // RECORDING THE FORWARD SWING IMPACT DEVIATION
