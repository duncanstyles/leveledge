 ...
 * FILE: Firmware.ino
 * VERSION: 5.2.11 (Cascade Shock & BLE Cache Bypass)
 * ===================================================================================== */
...
void setup() {
  ...
  // --- CASCADE SHOCK ---
  // Tier 1: The Slam (Violent correction)
  filter.setBeta(10.0f);
  for(int i=0; i<2000; i++) filter.updateIMU(0, 0, 0, snapAx, snapAy, snapAz);
  
  // Tier 2: The Brakes (Catch overshoot)
  filter.setBeta(2.0f);
  for(int i=0; i<2000; i++) filter.updateIMU(0, 0, 0, snapAx, snapAy, snapAz);
  
  // Tier 3: The Polish (Absolute zero-in)
  filter.setBeta(0.2f);
  for(int i=0; i<2000; i++) filter.updateIMU(0, 0, 0, snapAx, snapAy, snapAz);
  
  filter.setBeta(0.01f); // Return to standard speed

  setLEDColor(0, 0, 255); delay(300); setLEDColor(0, 0, 0); 
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (commandChar.written()) {
        ...
        if (len > 0) {
            if (data[0] == 82) { // 'R'
                ...
                gyroBiasX /= 200.0;
                gyroBiasY /= 200.0; gyroBiasZ /= 200.0;
                ...
                minPitch = 0.0; maxPitch = 0.0; zVelocity = 0.0;
                peakPush = 0.0; 
            ...
            else if (data[0] == 67 && len >= 8) { // 'C' (Padded Apple/Mac packets allowed)
                ...
                // BLE CACHE TRAP FIX: Fire a blank packet to reset the radio's echo blocker
                strikeChar.writeValue((const uint8_t*)" ", 1);
                delay(5); // Tiny pause to let the BLE stack breathe
                
                // Fire the true version handshake
      ...
      if (currentMicros - previousImuMicros >= IMU_INTERVAL_MICROS) {
        ...
        ay = rawAx; 
        az = -rawAz;
        float gx = rawGy - gyroBiasX; 
        float gy = rawGx - gyroBiasY;
        ...
        if (isArmed) {
            ...
            if (!inImpactWindow && deltaG >= impactThreshold && (currentMillis - lastImpactTime > 500)) {
                inImpactWindow = true; impactStartTime = currentMillis;
        ...
        if (inImpactWindow) {
            if (currentMillis - impactStartTime <= 25) {
                ...
                inImpactWindow = false; lastImpactTime = currentMillis;
                ...
                StrikePacket sp;
                sp.header = 83; sp.peakG = (int16_t)(impactPeakG * 100.0); sp.peakTwist = (int16_t)(impactPeakTwist * 10.0);
